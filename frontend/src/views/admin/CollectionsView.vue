<template>
  <div>
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
      <div>
        <h1 class="text-2xl font-sans font-bold dark:text-white">Collections</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Group projects into collections</p>
      </div>
    ...
    <div class="glass-card !p-4 mb-6">
      <div class="relative max-w-md">
        ...
        <input v-model="search" type="text" placeholder="Search collections…" class="input-field !pl-10 w-full" @keyup.enter="fetchList(1)" />
      </div>
    </div>
    ...
    <div v-else class="glass-card overflow-x-auto">
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-gray-200 dark:border-dark-600 text-left text-gray-500 dark:text-gray-400">
            ...
            <th class="py-3 px-4 font-medium">Projects</th>
        ...
        <tbody>
          <tr v-for="row in rows" :key="row.id" class="border-b border-gray-100 dark:border-dark-600/80 hover:bg-gray-50/80 dark:hover:bg-white/[0.02]">
            <td class="py-3 px-4">
              <router-link
                :to="{ name: 'public-collection', params: { username: row.author?.username || 'user', slug: row.slug } }"
            ...
            <td class="py-3 px-4">
              <router-link
                :to="{ name: 'admin-projects', query: { collection_id: row.id } }"
      ...
      <div v-if="!rows.length" class="text-center py-12 text-gray-500 text-sm">No collections yet.</div>
      ...
      <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-150" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="modalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @click.self="modalOpen = false">
          <div class="w-full max-w-md rounded-2xl bg-white dark:bg-dark-800 border border-gray-200 dark:border-dark-600 shadow-xl p-6 space-y-4">
            <h2 class="text-lg font-bold dark:text-white">{{ editingId ? 'Edit Collection' : 'New Collection' }}</h2>
            <div>
      ...
      <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-150" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="deleteTarget" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @click.self="deleteTarget = null">
          <div class="w-full max-w-sm rounded-2xl bg-white dark:bg-dark-800 border border-gray-200 dark:border-dark-600 shadow-xl p-6 text-center">
            <p class="text-gray-700 dark:text-gray-200 mb-4">Delete collection <strong>{{ deleteTarget.name }}</strong>? Projects are kept and become uncategorized.</p>
...
function slugify(t) {
  return String(t || '').toLowerCase().replace(/[^\w\s-]/g, '').replace(/[\s_]+/g, '-').replace(/^-+|-+$/g, '') || 'collection'
}
...
async function fetchList(page = 1) {
  loading.value = true
  try {
    const { data } = await adminApi.getCollections({ page, search: search.value || undefined })
    rows.value = data.data || []
    meta.value = data.meta || meta.value
  } catch {
    rows.value = []
    uiStore.showToast('Failed to load collections', 'error')
  } finally {
    loading.value = false
  }
}
...
async function saveModal() {
  ...
  try {
    if (editingId.value) {
      await adminApi.updateCollection(editingId.value, { ...form.value })
      uiStore.showToast('Collection updated')
    } else {
      await adminApi.createCollection({ ...form.value })
...
async function doDelete() {
  ...
  deleting.value = true
  try {
    await adminApi.deleteCollection(deleteTarget.value.id)
    uiStore.showToast('Collection deleted')
